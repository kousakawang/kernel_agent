...
Runtime instrumentation 优先根据 `target.file` 推导可导入模块，在模块执行完成且 import
尚未返回调用方时 wrap `qualified_name` 对应的函数。该 patch 只修改目标 Python 进程内的
module/class attribute，不修改磁盘源码。直接执行的 `__main__`、不可导入布局或属性定位失败时，
回退到轻量化 `sys.setprofile`；因此 profiler 是兼容兜底而不是默认路径。
...
| `test_cmd` | 唯一的 high-level target 触发命令；无服务时需可重复执行，CLI 先预热一次再正式采集一次 |
...
finalize → kernel-interface-decomposition/v3
...
- 所有模式统一为“launch → warmup/ready → nsys start → test → nsys stop”；无服务时由
