...
用户输入一个 `source-locate-agent-config/v1`，其中引用：

- KID 最终 schema；
- `third_party_manifest.json`；
- SGLang 源码根；
- 当前 testcase 独占 workspace。

入口 Prompt 内部执行 locate CLI 生成带临时 `locate_candidates.interface_definition` 的 candidate
schema，并使用 SGLang、sgl-kernel、third-party 源码及 build metadata 完成后续搜索。
...
- extract 后的 schema、`kernel_sources/` 与 `read_hints.txt`。

`locate/extract` 仍是两个公开 CLI，但由统一入口 Prompt 内部调用；用户不再手工串联阶段。
...
- `kernel_agent/example_kernels/nsys_poc_kid_golden/nsys_poc/output/decomposition.schema.json`
