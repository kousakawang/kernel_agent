...
1. **KID 增强**（`tools/kernel_interface_decomposer` 现有工具升级）：
...
#### 8.1.1 `resolve-third-party` skill

**类型**：Skill（agent 独立上下文运行；不做 CLI）

**位置**

- 主文件：`framework_engineer/skills/resolve_third_party.md`（skill 入口）
- helper 包：`framework_engineer/third_party_solver/`（版本解析 / clone 等 deterministic 逻辑）

**输入**（配置文件）

- N 条 sglang 启动命令 + 测试命令
- `sglang_repo_root`（含 `sgl-kernel/` 源码树）
- `third_party_cache`（clone 目的路径，按 `(name, version)` 分目录）
- `explicit_paths`（可选，用户手填的 P1 路径）

**输出**

- `<output_root>/third_party_manifest.json`
- `<output_root>/missing_repos.md`（若有 FAILED 项）
- `third_party_cache/<name>/<version>/` 下已 clone 的仓库

##### 点 1：需要哪些 third-party —— 固定 universe + flag 裁剪

依赖集合有限，**做固定注册表（universe）而非现场发现**；启动命令 flag 只用来裁剪本次实际 clone 的范围。

固定 universe（GPU 推理相关）：

```
flashinfer  flash_attn(sgl-attn)  flash_mla(FlashMLA)  deep_gemm
fla  causal_conv1d  cutlass  mscclpp  triton_kernels
```

每条预登记 `{ dist_name, archetype(F*), version_source, clone_url_source }`。两个不能省的补充：

1. **同名多版本 → 缓存 key 必须是 `(name, version)`**：如 `flashinfer` 同时以 F3（编进 sgl-kernel 的 `norm.cu`，pin `@bc29697`）和 F7（运行时 GDN/attention，pip `0.6.12`）存在，是两个 commit。
2. **flag 裁剪 clone**：解析 `--attention-backend` / `--linear-attn-backend` / `--moe-runner-backend` 等标注每个库属于哪条 backend 路径，只 clone 命中的（cutlass / sgl-attn 是大仓）。读不准就全 clone 或交人工，**不猜漏**。

##### 点 2：如何确定各仓库版本（严格二分）

版本源**必须按 archetype 二分**，不能用单一「runtime 版本」覆盖。这一步 + clone 完成，Step 0.5 的仓库准备即完成。

**Bucket A —— 独立 pip 包（运行时加载）→ `importlib.metadata`**
适用 F6/F7 及独立 F5：`flashinfer` / `sgl-deep-gemm` / `fla` / `flash-attn-4` / 独立装的 `causal_conv1d`。

```python
importlib.metadata.version("flashinfer_python")   # -> "0.6.12"  权威
```

以**装好的**为准，不读 pyproject 声明（如 `flash-attn-4>=4.0.0b9` 是范围）。clone URL = 官方源 + 该版本 tag。

**Bucket B —— 编进 `.so`、无独立版本 → 读 sgl-kernel 构建 pin**
适用 F3：`sgl-attn` / `FlashMLA` / `cutlass` / `flashinfer(norm/renorm)` / `mscclpp` / `triton_kernels`。

关键断层：dist-packages 里的 `sgl_kernel` wheel **只有 `.so`，没有 CMakeLists**，`version.py` 只给 `0.4.3`，这个数字本身不含 fetch commit。完整 4 步链：

```
1. importlib.metadata.version("sgl_kernel")   -> 0.4.3          # 运行时真实版本（锚点）
2. 用 0.4.3 对齐 sgl-kernel 源码树                                 # 校验 sglang_repo_root/sgl-kernel/version.py == 0.4.3；
                                                                #   不一致则 checkout 匹配 tag（否则 pin 对不上）
3. 读该源码树的 FetchContent pin：
     CMakeLists.txt:        sgl-attn@bcf72cc / flashinfer@bc29697 / cutlass@57e3cfb / mscclpp / triton v3.6.0
     cmake/flashmla.cmake:  FlashMLA@df022eb
4. 按 (commit + SHA256) shallow clone
```

FetchContent 的 URL 直接写明 `sgl-project/sgl-attn` vs `NVIDIA/cutlass`，即**「sgl fork 还是官方」不用猜**。

**Clone 策略**：`git clone --depth=1`；Bucket A 用 `--branch=v{pinned}`，tag 不存在 fallback commit hash；Bucket B 直接 checkout commit；`(name, version)` 缓存命中跳过。
...
```json
{
  ...
  "archetype": "F6",
  "version": "0.2.7",
  "version_source": "importlib.metadata | sgl_kernel_cmake_pin",
  "clone_source": "official | sgl_fork | embedded | installed",
  "local_path": "third_party_cache/fla/0.2.7",
  "triggered_by": ["triton", "cutedsl"],
  "evidence": "可选：为什么选这个仓库/版本"
}
```

`name` + `version` + `local_path` 必填；`evidence` 可选。

**Help function（参考，开发者自定）**

- `resolve_universe(service_cmds) -> list[RepoNeed]`：固定 list + flag 裁剪
- `pip_version(dist_name) -> str`（Bucket A）
- `sgl_kernel_fetch_pins(sgl_kernel_src) -> dict[name, (url, commit)]`（Bucket B，解析 CMakeLists / flashmla.cmake）
- `align_sgl_kernel_src(installed_version, sgl_kernel_src) -> Path`：版本对齐/校验
- `shallow_clone(url, ref, dest) -> Path`

**约束**

- 严格禁止修改 sglang 源码、重编 sgl-kernel、装新包；clone 仅作参考，不链接进环境
- F8（下载 cubin）不 clone，直接 FAILED 进 `missing_repos.md`
- 允许打 log / print 调试，skill 结束前清理

**依赖**：无（独立第一步）

**测试**（Qwen3.5 GDN，三条 backend：triton / flashinfer / cutedsl）

1. `fla` / `causal_conv1d`：Bucket A 定版本，`version` 与运行环境一致，`triggered_by` 正确
2. `flash_attn(sgl-attn)` / `flash_mla`：Bucket B，`clone_source=sgl_fork`，commit == CMake pin
3. `flashinfer` 出现两条 `(name, version)`（F3 编译态 + F7 运行态），互不覆盖
4. 每个 `local_path` 可读；F8 项进 `missing_repos.md`
...
**信息源优先级**：第一层 helper **优先消费 KID 运行时 event**，event 缺失才 fallback 到静态解析（机制①/②）。KID 的 [runtime_instrumentation.py](file:///Users/bytedance/Desktop/infra_agent/kernel_agent/tools/kernel_interface_decomposer/runtime_instrumentation.py) 已 patch `load_jit`，对每个接口记录：
...
**类型**：现有 `tools/kernel_interface_decomposer/` 增强 + 新增 CLI

**位置**

- KID 增强：`tools/kernel_interface_decomposer/` 现有模块
